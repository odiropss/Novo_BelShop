...
uses
  Forms, SysUtils, Classes, DBXpress, DB, SqlExpr, FMTBcd;

type
  TDMNFeAnaliza = class(TDataModule)
    SQLC: TSQLConnection;
    SQLQuery1: TSQLQuery;
...
var
  DMNFeAnaliza: TDMNFeAnaliza;

  sgPath_Local, sgPastaXMLArquivei,
  sgTpConexao   : String;
...
Procedure TDMNFeAnaliza.ConectaBanco;
Var
  Arq: TextFile;
  s, sBancoIB: String;
  i: Integer;
...
// Tipo de Conexão >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
Procedure TDMNFeAnaliza.BuscaTipoConexao;
Var
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        If i=3 Then sgPastaXMLArquivei:=IncludeTrailingPathDelimiter(Trim(s));
      End;
...
procedure TDMNFeAnaliza.DataModuleCreate(Sender: TObject);
Var
  i: Integer;
  Arq: TextFile;
