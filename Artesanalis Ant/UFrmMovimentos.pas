...
// Inclução/Exclução do Documento >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
End; // Inclução/Exclução do Documento >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
begin
  ...
  If bValidou Then
  Begin
    // Update or Insert Pessoa =================================================
    If DMLPessoas(sNome, sCnpjCpf) Then
    Begin
      // Busca o Código Depois de Update or Insert
      MySql:=' SELECT p.cod_pessoa'+
             ' FROM PESSOAS p'+
             ' where p.num_cnpjcpf='+QuotedStr(sCnpjCpf);
      DMArtesanalis.CDS_BuscaRapida.Close;
      DMArtesanalis.SQLQ_BuscaRapida.SQL.Clear;
      DMArtesanalis.SQLQ_BuscaRapida.SQL.Add(MySql);
      DMArtesanalis.CDS_BuscaRapida.Open;
      sNome:=DMArtesanalis.CDS_BuscaRapida.FieldByName('Cod_Pessoa').AsString;
      DMArtesanalis.CDS_BuscaRapida.Close;

      If Trim(sNome)<>'' Then
      Begin
        EdtCodPessoa.AsInteger:=StrToInt(sNome);
        EdtCodPessoaExit(Self);
      End;
    End; // If DMLPessoas(sNome, sCnpjCpf) Then
  End; // If bValidou Then
end;
