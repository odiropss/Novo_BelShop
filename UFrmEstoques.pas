...
Begin
  ...
  Begin
    ...
    // Se Tem Produto N�o Busca Pelo Fornecedor
...
begin
  ...
  AddToolTip(Pan_EstoquesProdNovo.Handle, @ti, TipoDoIcone, 'Cadastrado a'+#13+'MENOS de um M�S !!', 'PRODUTO NOVO !!');
...
begin
  ...
  If Bt_EstoquesDemonstrativo.Caption='Abrir Demonstrativo' Then
   Begin
     ...
     // Libera para Salvar em Mem�ria ==========================================
     Bt_EstoquesFiltroComprador.Caption:='Salvar em Mem�ria';

     // Abre Apresenta��o Fisico / Financeiro ==================================
     ...
     FrmSolicitacoes.Ts_QualquerCoisa.Caption:='F�sico/Financeiro';
...
begin
  ...
  // Retorna Valores Originais se N�o Houve Filtro =============================
  ...
  If DMVirtual.CDS_V_Estoques.IsEmpty Then
  Begin
    msg('Sem Produto a Listar !!'+cr+cr+'Filtro Ser� Retirado !!','A');
    DMVirtual.CDS_V_Estoques.Close;
    DMBelShop.CDS_EstoquePrevisao.Close;
...
begin
  ...
  // Obs para Utiliza��o no Movto de Comprovantes
  ...
  // Filtor nao Produtos de N�o Compra
  ...
  //-----------------------------------------------------------------
  // Filtro de Aplica��es dos Produtos ------------------------------
  //-----------------------------------------------------------------
  FrmBelShop.TS_FiltroAplicacao.TabVisible:=False;

  //-----------------------------------------------------------------
  // Filtro de Familia de Pre�os ------------------------------------
  //-----------------------------------------------------------------
...
Begin
  ...
  Begin
    msg('� Permitido Somente uma Loja por Vez !!','A');
    Dbg_Estoques.SetFocus;
    Exit;
  ...
  MySqlSelect:=
         ...
         '      WHEN pr.situacaopro=3 THEN ''N�o Compra'''+
         '      WHEN pr.situacaopro=4 THEN ''N�o Venda'''+
         '      ELSE ''Sem Informa��o'''+
         '   END'+
         ...
         ' 0.00 VLR_VENDAS_ACUM,'+ // N�o Usado
         ' GEN_ID('+sGenOrdem+',1)+12 ORDENAR,'+
         ' ''NAO'' ALTERACAO,'+
         ' 0.00 VLR_VENDAS_ACUM_OK,'+ // N�o Usado
...
begin
  ...
  Begin
    msg('Op��o Somente para o'+cr+cr+'Administrador do Sistema !!','A');
    Exit;
  End;
  ...
  FrmSolicitacoes.Caption:='Par�metros';
  ...
  // Transfere Painel dos Parametros da Curva ABC para Form de Solicita��es ====
...
Var
  ...
  iIndex: Integer;
begin
  // Altera os Estoques M�nimos ================================================
  If DMVirtual.CDS_V_Estoques.IsEmpty Then
   Exit;

  If Trim(sgFiltros)='' Then
  Begin
    If msg('Todos os FORNECEDORES EST�O Selecionados ??'+cr+cr+'DESEJA CONTINUAR ??','C')=2 Then
     Exit;
  End;
  ...
  If msg('Deseja Realmente Alterar O ESTOQUE M�NIMO'+cr+cr+'DE TODOS OS PRODUTOS SELECIONADOS ??','C')=2 Then
   Exit;

  // Solicita o Estoque M�nimo =================================================
  sEstMinimo:='';
  b:=True;
  While b do
  Begin
    sEstMinimo:=InputBoxInteiro('Estoque M�nimo de Todos os Produtos Selecionados', 'Informe o Novo Est. M�nimo',sEstMinimo);
    ...
    Try
      ...
      If msg('O NOVO Estoque M�nimo: '+sEstMinimo+cr+cr+'Esta CORRETO ??','C')=2 Then
       Exit;
...
Var
  MySql: String;
begin
  // Altera os Estoques M�nimos em Outras Lojas ================================
  If DMVirtual.CDS_V_Estoques.IsEmpty Then
   Exit;
  ...
  If Application.MessageBox('Deseja REALMENTE Replicar'+cr+cr+'TODOS OS ESTOQUE M�NIMOS'+cr+cr+'do FILTRO Selecionado para'+cr+cr+'Outra(s) Loja(s) ??', 'ATEN��O !!', 292) = Idno Then
   Exit;
  ...
  FrmSelectEmpProcessamento:=TFrmSelectEmpProcessamento.Create(Self);
  FrmSelectEmpProcessamento.bUsarMatriz:=False;
  FrmSelectEmpProcessamento.Gb_SelectEmpProc.Caption:='SELECIONE AS LOJAS A REPLICAR OS ESTOQUES M�NIMOS';
  ...
  If Application.MessageBox(pChar('Deseja REALMENTE Replicar'+cr+cr+'TODOS OS ESTOQUE M�NIMOS'+cr+cr+'do FILTRO Selecionado para'+cr+cr+'a(s) Loja(s):'+cr+cr+sgCodLojas+cr+cr+' ?????????'), 'ATEN��O !!', 292) = Idno Then
   Exit;

  OdirPanApres.Caption:='AGUARDE !! Replicando Estoques M�nimo...';
  OdirPanApres.Width:=Length(OdirPanApres.Caption)*10;
  OdirPanApres.Left:=ParteInteiro(FloatToStr((FrmEstoques.Width-OdirPanApres.Width)/2));
  OdirPanApres.Top:=ParteInteiro(FloatToStr((FrmEstoques.Height-OdirPanApres.Height)/2))-20;
  OdirPanApres.BringToFront();
...
begin
  ...
  If Bt_EstoquesFiltroComprador.Caption='Salvar em Mem�ria' Then
  Begin
    Dbg_EstoquesDemFinan.SetFocus;
  ...
  If Trim(sgFiltros)<>'' Then
  Begin
    If msg('Existe Filtro Anterior !!'+cr+cr+'DESEJA RETIR�-LO ??','C')=1 Then
     sgFiltros:='';
  End;
