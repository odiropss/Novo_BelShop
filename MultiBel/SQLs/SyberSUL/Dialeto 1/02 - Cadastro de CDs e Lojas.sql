...
CAST(CAST(REPLACE(REPLACE(REPLACE(COALESCE(l.ciccgc,'99999999999999'),
     '/', ''),'.',''),'-','') AS INTEGER) AS VARCHAR(18)) COD_LOJA, -- 3 CNPJ da Distribuidora
*/
...
CAST(REPLACE(REPLACE(REPLACE(COALESCE(c.ciccgc,'99999999999999'), '/', ''),'.',''),'-','') AS integer), -- 3 CNPJ da Distribuidora
