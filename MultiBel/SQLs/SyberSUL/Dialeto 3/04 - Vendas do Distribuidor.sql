...
CAST(CAST(REPLACE(REPLACE(REPLACE(COALESCE(fo.fcnpj_cic,'99999999999999'), '/', ''),'.',''),'-','')
    AS BIGINT) as VARCHAR(18)) CNPJ_FORN, -- 4 CNPJ do Fornecedor
 /*
CASE
...
CASE
   WHEN CHAR_LENGTH(TRIM(CAST(CAST(REPLACE(REPLACE(REPLACE(COALESCE(cl.cgc_cnpj,'99999999999999'),
                       '/', ''),'.',''),'-','') AS BIGINT) AS VARCHAR(18))))<12 Then
     1
   ELSE
     2
END  TIPO_IDENT_PDV, -- 15 Tipo Identificador PDV

TRIM(CAST(CAST(REPLACE(REPLACE(REPLACE(COALESCE(cl.cgc_cnpj,'99999999999999'),
                       '/', ''),'.',''),'-','') AS BIGINT) AS VARCHAR(18))) IDENTIFICADOR_PDV, -- 16 Identificador do PDV
